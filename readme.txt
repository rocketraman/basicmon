...
The timer and counter based on the java.util.concurrent.atomic classes is the default. This timer is generally a good
choice for most situations. The alternative synchronization-based timer and counter may be a better choice for some
concurrency scenarios, or on some platforms. Use BasicMonManager.getSyncTimer or BasicMonManager.getSyncCounter
to get an implementation based on java synchronization.

Synchronization is always used for timers/counters with more statistics turned on (min, max, etc.). I recommend using
javasimon rather than BasicMon if these additional statistics are required.
